...
module ConsoleGame
  module Chess
    ...
    module PieceLookup
      ...
      def group_fetch(query, choices:, t_data:, pieces: [])
      ...
      def fetch_all(side = :all, type:, t_data:)
      ...
      def reverse_lookup(side, type, target, file_rank = nil, available_pieces:, piece_lib:, alg_dict:)
        type = Chess.const_get(piece_lib.dig(type, :class))
        result = refined_lookup(fetch_all(side, type:), side, alg_dict.call(target), file_rank, available_pieces:)
      ...
      def refined_lookup(filtered_pieces, side, new_pos, file_rank, available_pieces:)
        filtered_pieces.select do |piece|
          next unless available_pieces[side].include?(piece.info)
